...
export function TaskListView({ columns, onTaskClick, onNewTask, onQuickAdd }: Props) {
  const [searchQuery,   setSearchQuery]   = useState('')
  const [quickTitle,    setQuickTitle]    = useState('')
  const [quickColId,    setQuickColId]    = useState<string>(columns[0]?.id ?? '')
  ...
  function submitQuickAdd() {
    ...
    if (!title || !quickColId || !onQuickAdd) return
    onQuickAdd(quickColId, title)
    setQuickTitle('')
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      {/* Quick-add bar */}
      {onQuickAdd && columns.length > 0 && (
        <div className="shrink-0 flex items-center gap-2 px-4 py-2 border-b border-border/60 bg-card/20">
          <input
            ref={quickInputRef}
            value={quickTitle}
            ...
            className="flex-1 h-7 bg-muted/30 rounded px-2.5 text-xs text-foreground placeholder:text-muted-foreground/35 focus:outline-none focus:ring-1 focus:ring-ring min-w-0"
          />
          <select
            value={quickColId}
            onChange={e => setQuickColId(e.target.value)}
            className="h-7 rounded border border-input bg-transparent px-2 text-xs text-foreground shrink-0 focus:outline-none focus:ring-1 focus:ring-ring"
          >
            {columns.map(col => (
          ...
          <button
            ...
            aria-label="Add task"
            className="p-1.5 rounded border border-border text-muted-foreground hover:text-foreground hover:bg-accent disabled:opacity-40 disabled:pointer-events-none transition-colors shrink-0"
          >
